...
protocol CarouselNotificationContentScene: AnyObject {
...
class OfflinePSDKScalingCarouselView: UICollectionView {
    ...
    convenience init(withFrame frame: CGRect, andInset inset: CGFloat) {
        self.init(
            frame: frame,
            collectionViewLayout: OfflineNotiveraScalingCarouselLayout(withCarouselInset: inset)
        )
        self.inset = inset
    }
    ...
    */
    ...
    func showNextItem() {
        guard let currentIndex = currentCenterCellIndex else {
            return
        }
        
        if currentIndex.row < (dataSource?.collectionView(self, numberOfItemsInSection: 0) ?? 0) - 1 {
            let newIndex = IndexPath(item: currentIndex.row + 1, section: currentIndex.section)
            scrollToItem(at: newIndex, at: .centeredHorizontally, animated: true)
        }
    }
...
private typealias PrivateAPI = OfflinePSDKScalingCarouselView
fileprivate extension PrivateAPI {
    ...
    func configureLayout() {
        ...
        collectionViewLayout = OfflineNotiveraScalingCarouselLayout(
 ...
 */
private typealias InvisibleScrollDelegate = OfflinePSDKScalingCarouselView
extension InvisibleScrollDelegate: UIScrollViewDelegate {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        */
        ...
        for cell in visibleCells {
            if let infoCardCell = cell as? OfflineCarouselNotificationContentCell {
...
extension OfflinePSDKScalingCarouselView: CarouselNotificationContentScene {
